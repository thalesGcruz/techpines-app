...
export default function FormSuggestSong({ onSubmit }: SuggestSongFormProps) {
  const {
    register,
    handleSubmit,
  ...
  async function submit(data: FormData) {
    try {
      await musicService.suggest(data);
      reset();
      toast.success("Sugestão enviada com sucesso!");
    } catch (err) {
      toast.error("Erro ao enviar sugestão, adicione uma url válida!");
      reset();
    }
  }
